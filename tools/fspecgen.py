...
# fspec_rv_no_memory
    ...
    def generate_cmkelists(self, cmakelists_path, custom):
        ...
        fprint(f'cmake_minimum_required(VERSION 3.15)')
        ...
        if self.spec.target_link_libraries:
            libs = ' '.join(self.spec.target_link_libraries)
            fprint(f'target_link_libraries({self.cmakelists_lib_name} PUBLIC {libs})')
...
class FuncProcessor:
    ...
    def generate_cmake_lists(self, fpath, has_registry=True):
        ...
        fprint('cmake_minimum_required(VERSION 3.15)')
        # fprint()
